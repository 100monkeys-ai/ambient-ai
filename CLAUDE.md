...
| `gateway/telegram*.py` | The second transport. `POST /webhook/telegram` in `telegram_webhook.py` (403 on a bad `X-Telegram-Bot-Api-Secret-Token`, 200 at once, mention detection: `@<bot_username>` from `getMe`, `@agent`, a reply to the bot, or any private-chat message), the Bot API client in `telegram.py` (`send_telegram`, `get_me`, `set_webhook`, `FAKE_TELEGRAM_OUTBOX`). Sender key is `tg:<user_id>` in the same `phone` column; portal links go to the user privately and fall back to the group when Telegram refuses. Startup sets the webhook only when `TELEGRAM_BOT_TOKEN` is set; the bot needs BotFather privacy mode disabled to see group messages. |
...
- **The LLM is Anthropic** (ruled by the architect 2026-09-12 20:10 UTC, after the OpenAI account returned `credit_balance_exhausted` and could not be funded before the demo). Two constants in `settings.py`, and nowhere else: `LLM_MODEL_ID = "claude-opus-5"` for the orchestrator, the Execution Agent, and synthesis, and `EXTRACTOR_MODEL_ID = "claude-haiku-4-5"` for the memory extractor, which is a bulk job off the reply path. The key is `ANTHROPIC_API_KEY`. `orchestration/llm.py` also holds the per-call model settings that keep the reply inside ADR-009's 30 seconds across four sequential calls: a small `max_tokens` on each (plan 1024, synthesis 512, extractor 1024) and `anthropic_effort="low"` on plan and synthesis. Thinking is never configured, so it stays adaptive; `budget_tokens` is rejected on these models.
